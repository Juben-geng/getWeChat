...
class FinalArticleCollector(ArticleCollector):
    """最终版文章收集器"""
    ...
    def create_document_with_content(
        ...
        # 构建并写入文档内容
        blocks = self._build_blocks(article, classification)
        ...
        for i, block in enumerate(blocks):
            try:
                ...
                result = self.feishu.request(
                    ...
                    {'children': [block], 'index': -1}  # 使用-1表示追加到末尾
                )
                ...
                if result.get('code') == 0:
                    ...
                    # 如果失败，打印错误信息
                    print(f"  警告: 块 {i+1} 写入失败 - {result.get('msg', '未知错误')}")
            except Exception as e:
                print(f"  警告: 块 {i+1} 写入异常 - {str(e)}")
        
        print(f"✓ 文档内容已写入 ({success_count}/{len(blocks)} 个块)")
    ...
    def _build_blocks(
        ...
        if article.get('msg_publish_time_str'):
            meta_parts.append(f"发布: {article['msg_publish_time_str'][:10]}")
        ...
        if summary:
            ...
            blocks.append({
                'block_type': 2,
                'text': {
                    'elements': [{'text_run': {'content': '📌 内容摘要：'}}]
                }
            })
            ...
            if len(summary) > 300:
                ...
                blocks.append({
                    'block_type': 2,
                    'text': {
                        'elements': [{'text_run': {'content': part1}}]
                    }
                })
                ...
                if part2:
                    blocks.append({
                        'block_type': 2,
                        'text': {
                            'elements': [{'text_run': {'content': part2}}]
                        }
                    })
            else:
                blocks.append({
        ...
        if content_text:
            blocks.append({
                'block_type': 2,
                'text': {
                    'elements': [{'text_run': {'content': '📖 正文片段：'}}]
                }
            })
            ...
            if len(paragraphs) > 3:
                blocks.append({
                    'block_type': 2,
                    'text': {
                        'elements': [{'text_run': {'content': f'... 还有 {len(paragraphs) - 3} 段内容，请查看原文'}}]
                    }
                })
    ...
    def collect_final(
        ...
        if mode in ['doc', 'both']:
            print("\n正在创建飞书文档...")
            doc = self.create_document_with_content(article, classification)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='最终版文章收集')
    ...
    collector = FinalArticleCollector()
    
    try:
        results = collector.collect_final(
            url=args.url,
            mode=args.mode,
